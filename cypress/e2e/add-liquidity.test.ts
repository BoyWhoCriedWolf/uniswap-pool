...
describe("Add Liquidity", () => {
  ...
  it("does not crash if token is duplicated", () => {
    cy.visit(
      "/add/0x1f9840a85d5aF5bf1D1762F925BDADdC4201F984/0x1f9840a85d5aF5bf1D1762F925BDADdC4201F984"
    );
    cy.get("#add-liquidity-input-tokena .token-symbol-container").should(
      "contain.text",
      "UNI"
  ...
  it("loads fee tier distribution", () => {
    cy.fixture("feeTierDistribution.json").then((feeTierDistribution) => {
      ...
      cy.get("#add-liquidity-selected-fee .selected-fee-label").should(
        "contain.text",
        "0.3% fee tier"
      );
      cy.get("#add-liquidity-selected-fee .selected-fee-percentage").should(
        "contain.text",
        "40% select"
      );
    });
  ...
  it("disables increment and decrement until initial prices are inputted", () => {
    ...
    cy.get('[data-testid="increment-price-range"]')
      .eq(0)
      .should("not.be.disabled");
    cy.get('[data-testid="decrement-price-range"]')
      .eq(0)
      .should("not.be.disabled");
