...
describe("swap flow logging", () => {
  it("completes two swaps and verifies the TTS logging for the first, plus all intermediate steps along the way", () => {
    ...
    cy.waitForAmplitudeEvent(SwapEventName.SWAP_FIRST_ACTION).then(
      (event: any) => {
        cy.wrap(event.event_properties).should(
          "have.property",
          "time_to_first_swap_action"
        );
        cy.wrap(event.event_properties.time_to_first_swap_action).should(
          "be.a",
          "number"
        );
        cy.wrap(event.event_properties.time_to_first_swap_action).should(
    ...
    // Verify Swap Quote
    cy.waitForAmplitudeEvent(SwapEventName.SWAP_QUOTE_FETCH).then(
      (event: any) => {
        ...
        if (event.event_properties.time_to_first_quote_request) {
          cy.wrap(event.event_properties.time_to_first_quote_request).should(
            "be.a",
            "number"
          );
          cy.wrap(event.event_properties.time_to_first_quote_request).should(
          ...
          cy.wrap(
            event.event_properties.time_to_first_quote_request_since_first_input
          ).should("be.a", "number");
          cy.wrap(
            event.event_properties.time_to_first_quote_request_since_first_input
          ).should("be.gte", 0);
    ...
    // Verify logging
    cy.waitForAmplitudeEvent(SwapEventName.SWAP_TRANSACTION_COMPLETED).then(
      (event: any) => {
        ...
        cy.wrap(event.event_properties.time_to_swap_since_first_input).should(
          "be.a",
          "number"
        );
        cy.wrap(event.event_properties.time_to_swap_since_first_input).should(
