...
function waitsForActiveChain(chain: string) {
  cy.get(getTestSelector("chain-selector-logo"))
    .invoke("attr", "alt")
    .should("eq", chain);
}
...
describe("network switching", () => {
  ...
  it("should not display message on user rejection", () => {
    ...
    cy.get(getTestSelector("popups")).should(
      "not.contain",
  ...
  it("should add missing chain", () => {
    cy.hardhat().then((hardhat) => {
      ...
      sendStub
        ...
        .callsFake(() =>
          added ? Promise.resolve(null) : Promise.reject(CHAIN_NOT_ADDED)
        );
