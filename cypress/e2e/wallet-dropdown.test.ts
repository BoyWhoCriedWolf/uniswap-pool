...
describe("Wallet Dropdown", () => {
  function itChangesTheme() {
    it("should change theme", () => {
      ...
      cy.get(getTestSelector("theme-lightmode")).should(
        "not.have.css",
        "background-color",
        "rgba(0, 0, 0, 0)"
      );
      cy.get(getTestSelector("theme-darkmode")).should(
        "have.css",
        "background-color",
        "rgba(0, 0, 0, 0)"
      );
      cy.get(getTestSelector("theme-auto")).should(
        "have.css",
        "background-color",
        "rgba(0, 0, 0, 0)"
      );
  ...
  function itChangesLocale({
    ...
    it("should change locale", () => {
      ...
      cy.get(getTestSelector("wallet-language-item"))
        .contains("Afrikaans")
        .click({ force: true });
  ...
  describe("should change locale with feature flag", () => {
    beforeEach(() => {
      cy.visit("/", {
        featureFlags: [{ name: FeatureFlag.currencyConversion, value: true }],
      });
      cy.get(getTestSelector("web3-status-connected")).click();
      cy.get(getTestSelector("wallet-settings")).click();
    });
  ...
  describe("testnet toggle", () => {
    ...
    it("should toggle testnet visibility", () => {
      ...
      cy.get(getTestSelector("chain-selector-options")).should(
        "not.contain.text",
        "Sepolia"
      );
  ...
  describe("disconnected", () => {
    beforeEach(() => {
      ...
      cy.get(getTestSelector("wallet-disconnect")).should(
        "contain",
        "Disconnect"
      );
      cy.get(getTestSelector("wallet-disconnect")).click();
  ...
  describe("with color theme", () => {
    ...
    it("should properly use dark system theme when auto theme setting is selected", () => {
      ...
      cy.get(getTestSelector("wallet-settings")).click();
      cy.get(getTestSelector("theme-auto")).click();
      cy.get(getTestSelector("wallet-header")).should(
        "have.css",
        "color",
        "rgb(155, 155, 155)"
      );
