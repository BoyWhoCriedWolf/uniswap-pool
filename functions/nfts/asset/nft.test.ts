const assets = [
  {
    ...
    image:
      "http://127.0.0.1:3000/api/image/nfts/asset/0xed5af388653567af2f388e6224dc7c4b3241c544/2550",
  },
  {
...
test.each(assets)("should inject metadata for valid assets", async (nft) => {
  const url =
    "http://127.0.0.1:3000/nfts/asset/" + nft.address + "/" + nft.assetId;
  const body = await fetch(new Request(url)).then((res) => res.text());
  expect(body).toMatchSnapshot();
  expect(body).toContain(
    `<meta property="og:title" content="${nft.collectionName} #${nft.assetId}"/>`
  );
  ...
  expect(body).toContain(
    `<meta property="og:image:alt" content="${nft.collectionName} #${nft.assetId}"/>`
  );
  expect(body).toContain(
    `<meta property="twitter:card" content="summary_large_image"/>`
  );
  expect(body).toContain(
    `<meta property="twitter:title" content="${nft.collectionName} #${nft.assetId}"/>`
  );
  expect(body).toContain(
    `<meta property="twitter:image" content="${nft.image}"/>`
  );
  expect(body).toContain(
    `<meta property="twitter:image:alt" content="${nft.collectionName} #${nft.assetId}"/>`
  );
});
...
test.each(invalidAssets)(
  "should not inject metadata for invalid asset calls",
  async (url) => {
    const body = await fetch(new Request(url)).then((res) => res.text());
