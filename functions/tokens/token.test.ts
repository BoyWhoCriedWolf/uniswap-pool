...
test.each(tokens)("should inject metadata for valid tokens", async (token) => {
  const url =
    "http://127.0.0.1:3000/tokens/" + token.network + "/" + token.address;
  const body = await fetch(new Request(url)).then((res) => res.text());
  expect(body).toMatchSnapshot();
  expect(body).toContain(
    `<meta property="og:title" content="Get ${token.symbol} on Uniswap"/>`
  );
  ...
  expect(body).toContain(
    `<meta property="og:image:alt" content="Get ${token.symbol} on Uniswap"/>`
  );
  expect(body).toContain(
    `<meta property="twitter:card" content="summary_large_image"/>`
  );
  expect(body).toContain(
    `<meta property="twitter:title" content="Get ${token.symbol} on Uniswap"/>`
  );
  expect(body).toContain(
    `<meta property="twitter:image" content="${token.image}"/>`
  );
  expect(body).toContain(
    `<meta property="twitter:image:alt" content="Get ${token.symbol} on Uniswap"/>`
  );
});
