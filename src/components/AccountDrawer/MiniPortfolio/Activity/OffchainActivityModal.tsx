...
export function useOpenOffchainActivityModal() {
  ...
  return useCallback(
    (order: { orderHash: string; status: UniswapXOrderStatus }) =>
...
const ContentContainer = styled(AutoColumn).attrs({
  justify: "center",
  gap: "md",
})`
...
function useOrderAmounts(
  ...
  const inputCurrency = useCurrency(
    orderDetails?.swapInfo?.inputCurrencyId,
    orderDetails?.chainId
  );
  const outputCurrency = useCurrency(
    orderDetails?.swapInfo?.outputCurrencyId,
    orderDetails?.chainId
  );
  ...
  if (!inputCurrency || !outputCurrency) {
    console.error(
      `Could not find token(s) for order ${orderDetails.orderHash}`
    );
    return undefined;
  }
...
export function OrderContent({ order }: { order: SelectedOrderInfo }) {
  ...
  const explorerLink = order?.details?.txHash
    ? getExplorerLink(
        order.details.chainId,
        order.details.txHash,
      ...
      return (
        <ContentContainer>
          ...
          <DescriptionText>
            ...
            <Trans>
              Your swap expired before it could be filled. Try again or
            </Trans>{" "}
      ...
      return (
        <ContentContainer>
          ...
          <ThemedText.LabelSmall textAlign="center">
            {/* TODO: Improve translation grammar by not having to break up the string */}
            <Trans>
              Your swap couldn&apos;t be filled at this time. Try again or{" "}
            </Trans>{" "}
