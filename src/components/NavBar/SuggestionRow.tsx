      ...
      <Row style={{ width: "60%" }}>
        {!brokenImage && collection.imageUrl ? (
          <Box
            ...
            alt={collection.name}
            className={clsx(
              loaded ? styles.suggestionImage : styles.imageHolder
            )}
        ...
        <Column className={styles.suggestionPrimaryContainer}>
          ...
          <Box className={styles.secondaryText}>
            {putCommas(collection?.stats?.total_supply ?? 0)} items
          </Box>
        </Column>
      </Row>
        ...
        <Column className={styles.suggestionSecondaryContainer}>
          <Row gap="4">
            <Box className={styles.primaryText}>
              {ethNumberStandardFormatter(collection.stats?.floor_price)} ETH
            </Box>
          </Row>
...
export const TokenRow = ({
  ...
  const handleClick = useCallback(() => {
    const address =
      !token.address && token.standard === TokenStandard.Native
        ? "NATIVE"
        : token.address;
  ...
  return (
    <Link
      data-testid={`searchbar-token-row-${token.chain}-${
            ...
            <PriceChangeContainer>
              ...
              <ThemedText.BodySmall>
                <DeltaText delta={token.market?.pricePercentChange?.value}>
                  {formatPercent(
                    Math.abs(token.market?.pricePercentChange?.value ?? 0)
                  )}
                </DeltaText>
              </ThemedText.BodySmall>
            </PriceChangeContainer>
