...
const StyledPollingBlockNumber = styled(ThemedText.DeprecatedSmall)<{
  ...
  color: ${({ theme, warning }) =>
    warning ? theme.deprecated_yellow3 : theme.success};
...
export default function Polling() {
  ...
  const waitMsBeforeWarning =
    (chainId
      ? getChainInfo(chainId)?.blockWaitMsBeforeWarning
      : DEFAULT_MS_BEFORE_WARNING) ?? DEFAULT_MS_BEFORE_WARNING;

  const warning = Boolean(
    !!blockTime &&
      machineTime - blockTime.mul(1000).toNumber() > waitMsBeforeWarning
  );
  ...
  return (
    <RowFixed>
      ...
      >
        ...
        >
          <ExternalLink href={blockExternalLinkHref}>
            <MouseoverTooltip
              text={
                <Trans>
                  The most recent block number on this network. Prices update on
                  every block.
                </Trans>
              }
            >
        ...
        <StyledPollingDot warning={warning}>
          {isMounting && <Spinner warning={warning} />}
        </StyledPollingDot>{" "}
      </StyledPolling>
