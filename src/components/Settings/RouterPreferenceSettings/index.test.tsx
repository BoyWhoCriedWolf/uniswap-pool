...
describe("RouterPreferenceSettings", () => {
  ...
  beforeEach(() => {
    store.dispatch(
      updateUserRouterPreference({ userRouterPreference: RouterPreference.API })
    );
  });
  it("toggles `Uniswap X` router preference", () => {
    ...
    expect(store.getState().user.userRouterPreference).toEqual(
      RouterPreference.X
    );
  ...
  it("toggles `Local Routing` router preference", () => {
    ...
    const localRoutingToggle = screen.getByTestId(
      "toggle-local-routing-button"
    );
