...
const getDeadlineInput = () =>
  screen.queryByTestId("deadline-input") as HTMLInputElement;
...
describe("TransactionDeadlineSettings", () => {
  describe("input", () => {
    ...
    beforeEach(() => {
      store.dispatch(
        updateUserDeadline({ userDeadline: DEFAULT_DEADLINE_FROM_NOW })
      );
    });
    ...
    it("is expanded by default when custom deadline is set", () => {
      store.dispatch(
        updateUserDeadline({ userDeadline: DEFAULT_DEADLINE_FROM_NOW * 2 })
      );
      renderTransactionDeadlineSettings();
