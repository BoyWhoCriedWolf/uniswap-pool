...
jest.mock("connection", () => {
  ...
  const { initializeConnector, MockEIP1193Provider } =
  ...
  const [connector, hooks] = initializeConnector(
    (actions: any) => new EIP1193({ actions, provider })
  );
  ...
  describe("analytics", () => {
    ...
    it("sends event when the active account changes", async () => {
      ...
      act(() => {
        ...
        mockProvider.emitAccountsChanged([
      ...
      expect(
        first(mocked(sendAnalyticsEvent).mock.invocationCallOrder)
      ).toBeGreaterThan(last(mocked(user.set).mock.invocationCallOrder));
      expect(
        first(mocked(sendAnalyticsEvent).mock.invocationCallOrder)
      ).toBeGreaterThan(last(mocked(user.postInsert).mock.invocationCallOrder));
    });
