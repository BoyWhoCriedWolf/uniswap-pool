...
export function GasBreakdownTooltip({
  trade,
  hideUniswapXDescription,
}: GasBreakdownTooltipProps) {
  ...
  const description =
    isUniswapX && !hideUniswapXDescription ? (
      ...
      <NetworkFeesDescription native={native} />
    );
...
function NetworkFeesDescription({ native }: { native: Currency }) {
  return (
    <ThemedText.LabelMicro>
      <Trans>
        The fee paid to the Ethereum network to process your transaction. This
        must be paid in {native.symbol}.
      </Trans>{" "}
