...
function useCollectionQuerySearch(
  query: string,
  skip?: boolean
): useCollectionSearchReturnProps {
        ...
        queryData?.nftCollections?.edges
          ?.filter(
            (collectionEdge) =>
              ...
              !blocklistedCollections.includes(
                collectionEdge.node.nftContracts?.[0]?.address
              )
          )
          ...
          .map((collectionEdge) => {
            const queryCollection =
              collectionEdge.node as NonNullable<NftCollection>;
