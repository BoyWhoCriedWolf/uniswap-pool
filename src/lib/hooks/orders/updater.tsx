...
function fetchOrderStatuses(account: string, orders: UniswapXOrderDetails[]) {
  ...
  return global.fetch(
    `${UNISWAP_API_URL}/orders?swapper=${account}&orderHashes=${orderHashes}`
  );
}
...
interface UpdaterProps {
  ...
  onOrderUpdate: (
    ...
    backendUpdate: UniswapXBackendOrder
  ) => void;
}

export default function OrderUpdater({
  pendingOrders,
  onOrderUpdate,
}: UpdaterProps): null {
  ...
  useEffect(() => {
    async function getOrderStatuses() {
      ...
      try {
        const pollOrderStatus = await fetchOrderStatuses(
          account,
          pendingOrders
        );
        ...
        pendingOrders.forEach((pendingOrder) => {
          const updatedOrder = orderStatuses.orders.find(
    ...
    const interval = setInterval(
      getOrderStatuses,
      OFF_CHAIN_ORDER_STATUS_POLLING
    );
