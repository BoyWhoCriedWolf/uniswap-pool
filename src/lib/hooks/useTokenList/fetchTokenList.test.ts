...
describe("fetchTokenList", () => {
  ...
  it("fetches and validates a list from an ENS address", async () => {
    ...
    const translatedUri =
      "https://cloudflare-ipfs.com/ipfs/QmPgEqyV3m8SB52BS2j2mJpu9zGprhj2BGCHtRiiw2fdM1/";
    resolver.mockResolvedValue(contenthash);
    fetch.mockOnceIf(translatedUri, () =>
      Promise.resolve(JSON.stringify(defaultTokenList))
    );
    await expect(fetchTokenList(url, resolver)).resolves.toStrictEqual(
      defaultTokenList
    );
  });
  ...
  it("fetches and validates the default token list", async () => {
    fetch.mockOnceIf(UNI_LIST, () =>
      Promise.resolve(JSON.stringify(defaultTokenList))
    );
    await expect(fetchTokenList(UNI_LIST, resolver)).resolves.toStrictEqual(
      defaultTokenList
    );
    expect(resolver).not.toHaveBeenCalled();
  });
