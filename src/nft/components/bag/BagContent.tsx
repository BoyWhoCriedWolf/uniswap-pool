...
} from "nft/components/bag/BagRow";
...
export const BagContent = () => {
  ...
  const setDidOpenUnavailableAssets = useBag(
    (s) => s.setDidOpenUnavailableAssets
  );
  ...
  const {
    ...
    const unchangedAssets = itemsInBag
      .filter(
        (item) =>
          item.status === BagItemStatus.ADDED_TO_BAG ||
          item.status === BagItemStatus.REVIEWED
      )
      .map((item) => item.asset);
    ...
    const unavailableAssets = itemsInBag
      ...
      .map((item) => item.asset);
    const availableItems = itemsInBag.filter(
      (item) => item.status !== BagItemStatus.UNAVAILABLE
    );
  ...
  return (
    <>
      <Column
        display={
          priceChangedAssets.length > 0 || unavailableAssets.length > 0
            ? "flex"
            : "none"
        }
      >
