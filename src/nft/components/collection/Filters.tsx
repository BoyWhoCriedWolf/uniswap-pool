...
export const Filters = ({
  ...
  traitsByGroup: Record<string, Trait[]>;
}) => {
  ...
  const [buyNowHovered, toggleBuyNowHover] = useReducer(
    (state) => !state,
    false
  );
    ...
    <Box className={styles.container}>
      ...
      <Column marginTop="8">
        ...
        >
          ...
          <Checkbox
            hovered={buyNowHovered}
            checked={buyNow}
            onClick={handleBuyNowToggle}
          >
            <span />
          </Checkbox>
        </Row>
        {isMobile && (
          <FilterSortDropdown sortDropDownOptions={sortDropDownOptions} />
        )}
        <MarketplaceSelect />
        <PriceRange />
        ...
        <Column>
          {Object.entries(traitsByGroup).map(([type, traits], index) => (
            ...
            <TraitSelect
              key={type}
              {...{ type, traits }}
              index={index + TraitPosition.TRAIT_START_INDEX}
            />
          ))}
        </Column>
      </Column>
