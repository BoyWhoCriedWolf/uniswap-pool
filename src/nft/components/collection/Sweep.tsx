...
const NftHolder = styled.div<{ index: number; src?: string }>`
  ...
  opacity: ${({ src, index }) =>
    src ? 1.0 : index === 0 ? 0.9 : index === 1 ? 0.6 : 0.3};
  transform: ${({ index }) =>
    index === 0
      ...
      {[...Array(3)].map((_, index) => {
        return (
          <NftHolder
            ...
            src={
              nfts.length - 1 >= index
                ? nfts[nfts.length - 1 - index].smallImageUrl
                : undefined
            }
          />
        );
...
export const Sweep = ({ contractAddress, minPrice, maxPrice }: SweepProps) => {
  ...
  const collectionParams = useSweepFetcherParams(
    contractAddress,
    "others",
    minPrice,
    maxPrice
  );
  const sudoSwapParams = useSweepFetcherParams(
    contractAddress,
    Markets.Sudoswap,
    minPrice,
    maxPrice
  );
  const nftxParams = useSweepFetcherParams(
    contractAddress,
    Markets.NFTX,
    minPrice,
    maxPrice
  );
  const nft20Params = useSweepFetcherParams(
    contractAddress,
    Markets.NFT20,
    minPrice,
    maxPrice
  ...
  const { sortedAssets, sortedAssetsTotalEth } = useMemo(() => {
    ...
    const sudoSwapAssetsInJointCollections = jointCollections.filter(
      (sweepAsset) =>
        sweepAsset.marketplace === Markets.Sudoswap && !sweepAsset.susFlag
    );
    ...
    jointCollections = collectionAssets
      ? collectionAssets.concat(jointCollections)
      : jointCollections;

    jointCollections.sort((a, b) => {
      return BigNumber.from(a.priceInfo.ETHPrice).gt(
        BigNumber.from(b.priceInfo.ETHPrice)
      )
        ? 1
        : -1;
    });
    ...
    let validAssets = jointCollections.filter(
      (asset) =>
        BigNumber.from(asset.priceInfo.ETHPrice).gt(0) && !asset.susFlag
    );
    ...
    if (sortedAssets) {
      if (isItemsToggled) {
        ...
        if (sweepItemsInBag.length < value) {
          addAssetsToBag(
            sortedAssets.slice(sweepItemsInBag.length, value),
            true
          );
        } else {
          removeAssetsFromBag(
            sweepItemsInBag.slice(value, sweepItemsInBag.length),
            true
          );
        }
        ...
        if (sweepEthPrice.lte(wishValueInWei)) {
          ...
          while (
            ...
            curTotal
              .add(BigNumber.from(sortedAssets[curIndex].priceInfo.ETHPrice))
              .lte(wishValueInWei)
          ) {
            wishAssets.push(sortedAssets[curIndex]);
            curTotal = curTotal.add(
              BigNumber.from(sortedAssets[curIndex].priceInfo.ETHPrice)
            );
            curIndex++;
          }
          ...
          while (curIndex >= 0 && curTotal.gt(wishValueInWei)) {
            ...
            curTotal = curTotal.sub(
              BigNumber.from(sweepItemsInBag[curIndex].priceInfo.ETHPrice)
            );
            curIndex--;
          }
    ...
    if (typeof value === "number") {
      if (sortedAssets) {
        if (isItemsToggled) {
          if (
            Math.floor(value) !==
            Math.floor(sweepAmount !== "" ? parseFloat(sweepAmount) : 0)
          )
        ...
        <SweepSubContainer>
          <StyledSlider
            ...
            max={
              isItemsToggled
                ...
                : parseFloat(formatEther(sortedAssetsTotalEth).toString())
            }
            value={
              isItemsToggled
                ? sweepItemsInBag.length
                : parseFloat(formatWeiToDecimal(sweepEthPrice.toString()))
            }
          ...
          <InputContainer
            ...
            value={
              isItemsToggled
                ? sweepAmount !== ""
                  ? Math.floor(parseFloat(sweepAmount))
                  : sweepAmount
                : sweepAmount
            }
            onChange={(event) => {
    ...
    if (isMarketFiltered) {
      if (market === "others") {
        const marketsToQuery = markets.filter(
          (market) => !isPooledMarket(market as Markets)
        );
