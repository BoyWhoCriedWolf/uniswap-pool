...
export const SortDropdown = ({
  ...
  const width = isCollectionStatsLoading
    ? 220
    : inFilters
    ? "full"
    : mini
      ...
      <Box
        ...
        className={
          isCollectionStatsLoading
            ? styles.isLoadingDropdown
            : clsx(isOpen && !mini && styles.activeDropdown)
        }
      >
        {!isCollectionStatsLoading && (
          <>
            <Box display="flex" alignItems="center" color="neutral1">
              {!isOpen && reversable && (
                <Row
                  ...
                  onClick={(e) => {
                    ...
                    if (dropDownOptions[selectedIndex].reverseOnClick) {
                      ...
                      const dropdownIndex =
                        dropDownOptions[selectedIndex].reverseIndex ?? 1;
                ...
                >
                  ...
                  {dropDownOptions[selectedIndex].reverseIndex &&
                    (selectedIndex >
                    (dropDownOptions[selectedIndex].reverseIndex ?? 1) - 1 ? (
                      <ArrowsIcon />
                    ) : (
                      <ReversedArrowsIcon />
                    ))}
              ...
              >
                {mini
                  ...
                  : isOpen
                  ? "Sort by"
                  : dropDownOptions[selectedIndex].displayText}
              </Box>
            </Box>
            <ChevronUpIcon
        ...
        {!maxWidth
          ? [
              dropDownOptions.reduce((acc, curr) => {
                return curr.displayText.length >= acc.displayText.length
                  ? curr
                  : acc;
              }, dropDownOptions[0]),
            ].map((option, index) => {
...
const LargestItem = ({
  ...
  useLayoutEffect(() => {
    if (maxWidthRef && maxWidthRef.current) {
      setMaxWidth(
        Math.ceil(maxWidthRef.current.getBoundingClientRect().width) +
          MAX_PADDING
      );
    }
  });
