...
const ContentColumn = styled(Column)<{ failed: boolean }>`
  background-color: ${({ theme, failed }) =>
    failed && opacify(12, theme.critical)};
  border-radius: 12px;
...
export const ContentRow = ({
  ...
  return (
    <ContentColumn failed={failed}>
      ...
      >
        {isCollectionApprovalSection ? (
          <CollectionIcon src={row.image} />
        ) : (
          <AssetIcon src={row.image} />
        )}
        <MarketplaceIcon>
          {getMarketplaceIcon(row.marketplace.name, "24")}
        </MarketplaceIcon>
        <ContentName>{row.name}</ContentName>
        {isCollectionApprovalSection && (row as CollectionRow).isVerified && (
          <StyledVerifiedIcon />
        )}
        <IconWrapper>
          {row.status === ListingStatus.DEFINED ||
