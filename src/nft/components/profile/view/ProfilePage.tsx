...
export const ProfilePage = () => {
  ...
  const walletCollections = useWalletCollections(
    (state) => state.walletCollections
  );
  const setWalletCollections = useWalletCollections(
    (state) => state.setWalletCollections
  );
  ...
  const {
    ...
    getNextPageParam: (lastGroup) =>
      lastGroup.data.length === 0 ? undefined : lastGroup.nextPage,
  ...
  const ownerCollections = useMemo(
    () =>
      isSuccess
        ? ownerCollectionsData?.pages.map((page) => page.data).flat()
        : null,
...
const ProfilePageNfts = ({
  ...
  const { address } = useWalletBalance();
  const setCollectionFilters = useWalletCollections(
    (state) => state.setCollectionFilters
  );
  const collectionFilters = useWalletCollections(
          ...
          <InfiniteScroll
            ...
            loader={
              Boolean(hasNext && ownerAssets?.length) && (
                <LoadingAssets count={DEFAULT_WALLET_ASSET_QUERY_AMOUNT} />
              )
            }
                  ...
                  <div key={index}>
                    <ViewMyNftsAsset
                      ...
                      mediaShouldBePlaying={
                        asset.tokenId === currentTokenPlayingMedia
                      }
