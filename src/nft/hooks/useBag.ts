    ...
    (set, get) => ({
      ...
      markAssetAsReviewed: (asset, toKeep) =>
        set(({ itemsInBag }) => {
          ...
          const index = itemsInBagCopy.findIndex(
      ...
      toggleBag: () =>
        set(({ bagExpanded }) => ({ bagExpanded: !bagExpanded })),
      usedSweep: false,
      isLocked: false,
      setLocked: (_isLocked) =>
        set(() => ({
          ...
          assets.forEach((asset) => {
            ...
            if (asset.tokenType !== NftStandard.Erc1155) {
              index = itemsInBag.findIndex(
                (n) =>
                  n.asset.tokenId === asset.tokenId &&
                  n.asset.address === asset.address
              );
            }
            if (index !== -1) {
      ...
      lockSweepItems: (contractAddress) =>
        set(({ itemsInBag }) => {
          ...
          const itemsInBagCopy = itemsInBag.map((item) =>
            item.asset.address === contractAddress && item.inSweep
              ? { ...item, inSweep: false }
              : item
          );
          if (itemsInBag.length === 0)
