        ...
        set(({ sellAssets }) => {
          ...
          assetsCopy.splice(
            sellAssets.findIndex(
              (n) =>
                n.tokenId === asset.tokenId &&
                n.asset_contract.address === asset.asset_contract.address
            ),
            1
          );
        ...
        set(({ sellAssets }) => {
          ...
          assetsCopy.map((asset) => {
            ...
            for (const marketplace of marketplaces) {
              ...
              listingIndex > -1
                ? (asset.newListings[listingIndex] = newListing)
                : asset.newListings.push(newListing);
            }
            return asset;
          });
          return { sellAssets: assetsCopy };
      ...
      removeAssetMarketplace: (asset, marketplace) => {
        set(({ sellAssets }) => {
          ...
          const marketplaceIndex =
            asset.marketplaces?.findIndex(
              (oldMarket) => oldMarket.name === marketplace.name
            ) ?? -1;
          const listingIndex = asset.newListings?.findIndex(
            (listing) => listing.marketplace.name === marketplace.name
          );
