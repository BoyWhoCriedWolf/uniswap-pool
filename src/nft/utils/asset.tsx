...
export const generateTweetForAsset = (asset: GenieAsset): string => {
  ...
  }%20(${asset.collectionName})%20https://app.uniswap.org/nfts/asset/${
    asset.address
...
export const generateTweetForPurchase = (
  ...
  const collectionUrl =
    assets.length > 0 && !multipleCollections
      ? `collection/${assets[0].address}`
      : "";
  ...
  const tweetText =
    assets.length == 1
      ? `I just listed ${
          ...
          .join(
            ", "
          )}. Buy it on @Uniswap at https://app.uniswap.org/${getAssetHref(
          assets[0]
        )}`
      : `I just listed ${
          assets.length
