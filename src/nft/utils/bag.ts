...
export function getPurchasableAssets(
  itemsInBag: BagItem[]
): UpdatedGenieAsset[] {
  return itemsInBag
...
export function getNextBagState(
  ...
  const { hasPriceAdjustment, updatedAssets } =
  ...
  const { unchanged, priceChanged, unavailable } =
  ...
  const newBagItems = createBagFromUpdatedAssets(
    unavailable,
    priceChanged,
    unchanged
  );
  const nextBagStatus = evaluateNextBagState(
    hasAssets,
    shouldReview,
    hasAssetsInReview,
