...
const urlParamsUtils = {
  removeDefaults: (query: Record<string, any>) => {
    ...
    for (const key in clonedQuery) {
      ...
      const initialValue =
        initialCollectionFilterState[
          key as keyof typeof initialCollectionFilterState
        ];
    ...
    const defaultSortByPointer =
      SortByPointers[initialCollectionFilterState.sortBy];
  ...
  buildQuery: (
    ...
    try {
      const { buyNow: initialBuyNow, search: initialSearchText } =
        initialCollectionFilterState;
      ...
      clonedQuery["buyNow"] = !(clonedQuery["all"] === undefined
        ? !initialBuyNow
        : clonedQuery["all"]);
      clonedQuery["search"] =
        clonedQuery["search"] === undefined
          ? initialSearchText
          : String(clonedQuery["search"]);
      ...
      if (clonedQuery["traits"].length === 2) {
        const [trait_type, trait_value] = clonedQuery["traits"] as [
          string,
          string
        ];
      ...
      clonedQuery["traits"] = clonedQuery["traits"].map(
        (queryTrait: string) => {
          ...
          const traitInStats =
            ...
            collectionStats.traits[trait_type].find(
              (trait) => trait.trait_value === trait_value
            );
...
export const syncLocalFiltersWithURL = (state: CollectionFilters) => {
  ...
  window.history.replaceState(
    {},
    ``,
    `${url}${stringifiedQuery && `?${stringifiedQuery}`}`
  );
};
