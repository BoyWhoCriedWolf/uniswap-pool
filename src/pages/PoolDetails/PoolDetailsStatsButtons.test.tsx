...
jest.mock(
  "components/AccountDrawer/MiniPortfolio/Pools/useMultiChainPositions"
);
...
describe("PoolDetailsStatsButton", () => {
  ...
  it("renders both buttons correctly", () => {
    ...
    expect(
      screen.getByTestId("pool-details-add-liquidity-button")
    ).toBeVisible();
  ...
  it("clicking swap goes to correct url", async () => {
    ...
    await act(() =>
      userEvent.click(screen.getByTestId("pool-details-swap-button"))
    );
  ...
  it("clicking add liquidity goes to correct url", async () => {
    ...
    await act(() =>
      userEvent.click(screen.getByTestId("pool-details-add-liquidity-button"))
    );
    expect(global.window.location.href).toContain(
