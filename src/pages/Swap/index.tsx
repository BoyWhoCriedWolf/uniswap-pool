...
export function Swap({
  ...
  const swapElement = (
    <SwapWrapper isDark={isDark} className={className} id="swap-page">
      <div style={{ backgroundColor: colors.dark_blue, padding: 8 }}>
        <TokenSafetyModal
        ...
        {!showOptInSmall && !isUniswapXDefaultEnabled && (
          <UniswapXOptIn isSmall={false} swapInfo={swapInfo} />
        )}
      </div>
      {showDetailsDropdown && (
        <SwapDetailsDropdown
  ...
  return (
    <>
      {swapElement}
      {showOptInSmall && !isUniswapXDefaultEnabled && (
        <UniswapXOptIn isSmall swapInfo={swapInfo} />
      )}
    </>
  );
}
