import {
  ...
  SwapEventType,
} from "./SwapEventTimestampTracker";
...
describe("SwapEventTimestampTracker", () => {
  ...
  it("should set and get elapsed time", () => {
    const instance = SwapEventTimestampTracker.getInstance();
    expect(instance.setElapsedTime(SwapEventType.FIRST_SWAP_SUCCESS)).toEqual(
      500
    );
    expect(instance.getElapsedTime(SwapEventType.FIRST_SWAP_SUCCESS)).toEqual(
      500
    );
  });
  ...
  it("should get elapsed time between two events", () => {
    const instance = SwapEventTimestampTracker.getInstance();
    expect(instance.setElapsedTime(SwapEventType.FIRST_SWAP_ACTION)).toEqual(
      100
    );
    expect(
      instance.getElapsedTime(
        SwapEventType.FIRST_SWAP_SUCCESS,
        SwapEventType.FIRST_SWAP_ACTION
      )
    ).toEqual(400);
  });
  ...
  it("should return undefined if event type not set", () => {
    const instance = SwapEventTimestampTracker.getInstance();
    expect(
      instance.getElapsedTime(SwapEventType.FIRST_QUOTE_FETCH_STARTED)
    ).toBeUndefined();
  });
});
