...
export function logSwapSuccess(
  ...
  const hasSetSwapSuccess = timestampTracker.hasTimestamp(
    SwapEventType.FIRST_SWAP_SUCCESS
  );
  const elapsedTime = timestampTracker.setElapsedTime(
    SwapEventType.FIRST_SWAP_SUCCESS
  );
...
export function logSwapQuoteRequest(
  ...
  if (routerPreference !== INTERNAL_ROUTER_PREFERENCE_PRICE) {
    const hasSetSwapQuote = timestampTracker.hasTimestamp(
      SwapEventType.FIRST_QUOTE_FETCH_STARTED
    );
    const elapsedTime = timestampTracker.setElapsedTime(
      SwapEventType.FIRST_QUOTE_FETCH_STARTED
    );
