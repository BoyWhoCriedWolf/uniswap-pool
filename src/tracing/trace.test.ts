...
function getTransaction(index = 0): Transaction {
  const transactions = mocked(Sentry.startTransaction).mock.results.map(
    ({ value }) => value
  );
...
describe("trace", () => {
  beforeEach(() => {
    mocked(Sentry.startTransaction).mockImplementation((context) => {
      const transaction: Transaction = jest
        .requireActual("@sentry/react")
  ...
  it("propagates callback", async () => {
    await expect(
      trace("test", () => Promise.resolve("resolved"))
    ).resolves.toBe("resolved");
    await expect(trace("test", () => Promise.reject("rejected"))).rejects.toBe(
      "rejected"
    );
  });
  ...
  describe("defaults status", () => {
    ...
    it('"internal_error" if rejected, with data.error set to rejection', async () => {
      ...
      await expect(trace("test", () => Promise.reject(error))).rejects.toBe(
        error
      );
      const transaction = getTransaction();
  ...
  describe("beforeSend", () => {
    it("handles no path", async () => {
      ...
      expect(
        (beforeSend(errorEvent, eventHint) as ErrorEvent)?.request?.url
      ).toEqual("https://app.uniswap.org");
    });
    ...
    it("handles hash with path", async () => {
      ...
      expect(
        (beforeSend(errorEvent, eventHint) as ErrorEvent)?.request?.url
      ).toEqual("https://app.uniswap.org/pools");
    });
