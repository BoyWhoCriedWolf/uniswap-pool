...
export function openDownloadApp(
  ...
  if (isIOS) {
    ...
    openWalletMicrosite({
      element: options?.element,
      urlParamString: options?.microSiteParams,
    });
  }
}
...
export const getDownloadAppLinkProps = (
  options: OpenDownloadAppOptions = defaultDownloadAppOptions
) => {
  return {
...
const openAppStore = (options?: AnalyticsLinkOptions) => {
  ...
  window.open(
    linkWithParams(APP_STORE_LINK, options?.urlParamString),
    /* target = */ "uniswap_wallet_appstore"
  );
};
...
export const openWalletMicrosite = (options?: AnalyticsLinkOptions) => {
  ...
  window.open(
    linkWithParams(MICROSITE_LINK, options?.urlParamString),
    /* target = */ "uniswap_wallet_microsite"
  );
};

const linkWithParams = (link: string, params?: string) =>
  link + (params ? `?${params}` : "");
