  ...
  describe("signTypedData", () => {
    ...
    function itFallsBackToEthSignIfUnimplemented(signingMethod: string) {
      it.each(["not found", "not implemented"])(
        ...
        async (message) => {
          ...
          expect(send).toHaveBeenCalledWith(signingMethod, [
            wallet,
            expect.anything(),
          ]);
          expect(send).toHaveBeenCalledWith("eth_sign", [
            wallet,
            expect.anything(),
          ]);
    ...
    function itFailsIfRejected(signingMethod: string) {
      it("fails if rejected", async () => {
        ...
        await expect(
          async () => await signTypedData(signer, domain, types, value)
    ...
    describe("wallets which do not support eth_signTypedData_v4", () => {
      describe.each(["SafePal Wallet", "Ledger Wallet Connect"])(
        "%s",
        (name) => {
          beforeEach(() => {
          ...
          it("signs using eth_signTypedData", async () => {
            ...
            await signTypedData(signer, domain, types, value);
            expect(send).toHaveBeenCalledTimes(1);
            expect(send).toHaveBeenCalledWith("eth_signTypedData", [
              wallet,
              expect.anything(),
            ]);
            const data = send.mock.lastCall[1]?.[1];
